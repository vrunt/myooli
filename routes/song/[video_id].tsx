...
export default async function SongPage(req: Request, ctx: FreshContext) {
  const { video_id } = ctx.params;
  const videoReq = await fetch(
    `https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails&id=${video_id}&key=${
      Deno.env.get("YT_API_KEY")
    }`,
  );
  ...
  const duration = videoData.contentDetails.duration.replace(
    /^PT(\d*?)H?(\d*?)M?(\d+)S?$/g,
    "$1h$2m$3s",
  );
  ...
  const renderListOfTags = (tags: Array<string>) => {
  ...
  return (
    <>
      ...
      <div>
        Description: <pre>{ videoData.snippet.description }</pre>
      </div>
    </>
  );
}
